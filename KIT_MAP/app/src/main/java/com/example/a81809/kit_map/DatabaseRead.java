...
public class DatabaseRead {
    ...
    public float[] getFloorRangeSize(int building_number, int floor) {
        String sql = "SELECT LeftLongitude FROM floor WHERE building_number = ? AND floor = ?";
        String[] where = {String.valueOf(building_number), String.valueOf(floor)};
        float[] result = new float[4];
        ArrayList<String> list = searchData(sql, where);
        result[0] = Float.parseFloat(list.get(0));
        sql = "SELECT TopLatitude FROM floor WHERE building_number = ? AND floor = ?";
        list = searchData(sql, where);
